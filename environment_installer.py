...
# Use DT_API_TOKEN_TESTING to create new, temporary, properly scoped API tokens
DT_API_TOKEN_FOR_USE = create_dt_api_token(token_name="[devrel e2e testing] DT_JMETER_E2E_TEST_TOKEN", scopes=["ReadConfig", "DataExport", "CaptureRequestData", "openpipeline.events_sdlc"], dt_rw_api_token=DT_API_TOKEN_TESTING, dt_tenant_live=DT_TENANT_LIVE)
...
if CODESPACE_NAME.startswith("dttest-"):
    ...
    run_command(["pip", "install", "-r", "requirements.txt", "--break-system-packages"], cwd=".devcontainer/testing")
    run_command(["python",  "testharness.py"], cwd=".devcontainer/testing")
